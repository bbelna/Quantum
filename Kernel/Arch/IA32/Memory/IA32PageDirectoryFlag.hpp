/**
 * @file Kernel/Arch/IA32/Memory/IA32PageDirectoryFlag.hpp
 * @brief Declares @ref @QKrnlIA32::Memory::IA32PageDirectoryFlag.
...
namespace Quantum::Kernel::Arch::IA32::Memory {
  /**
   * @brief IA-32 page directory entry bit flags.
   *
   * Each flag corresponds to a hardware-defined bit in the 32-bit page
   * directory entry. Bitmask operators are enabled via
   * `QUANTUM_ENABLE_ENUM_BITMASK_OPS` so flags can be combined with `|`
   * and tested with `HasAnyFlag`.
   */
  enum class IA32PageDirectoryFlag : UInt32 {
    /**
     * @brief No page flags set.
     */
    None = 0,
    ...
    /**
     * @brief Page size (4 MB if set in page directory).
     */
    PageSize = 1u << 7,
...
QUANTUM_ENABLE_ENUM_BITMASK_OPS(
  Quantum::Kernel::Arch::IA32::Memory, IA32PageDirectoryFlag
)
